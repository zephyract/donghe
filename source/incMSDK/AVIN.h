...
/**video signal mode.*/
typedef enum TVD_SIGNAL_MODE_E {
	AVIN_SIGNAL_PAL_N,     /**< ntsc mode signal*/
	AVIN_SIGNAL_PAL,       /**< PAL mode signal*/
	AVIN_SIGNAL_PAL_M,     /**< PAL_M mode signal*/
	AVIN_SIGNAL_NTSC,      /**< ntsc mode signal*/
	AVIN_SIGNAL_SECAM,     /**< secam mode signal*/
	AVIN_SIGNAL_PAL_60,    /**< PAL 60_M mode signal*/
	AVIN_SIGNAL_UNSTABLE,  /**< Represent video signal is not stable yet! In Hardware, it's reserved*/
	AVIN_SIGNAL_NTSC443,   /**< NTSC443 mode signal*/
...
typedef struct _AVIN_AUDIO_ATTRIBUTE_
{
    UINT32 u4Length;
}AVIN_AUDIO_ATTRIBUTE, *PAVIN_AUDIO_ATTRIBUTE;


typedef struct _AVIN_VIDEO_ATTRIBUTE_
{
	UINT32 u4Length;
	UINT32 u4SigMode;
	...
	BOOL   bInterlace;
	BOOL   bTopFieldFirst;
	UINT32 u4SourceWidth;
	UINT32 u4SourceHeight;
	UINT32 u4StartTopLine;
	UINT32 u4StartBottomLine;
	UINT32 u4ActiveLine;
	UINT32 u4VPolarity;
	UINT32 u4HPolarity;
	...
	UINT8  uDatas[1];
}AVIN_VIDEO_ATTRIBUTE, *PAVIN_VIDEO_ATTRIBUTE;

typedef struct _AVIN_APP_ATTRIBUTE_
{
    AVIN_VIDEO_ATTRIBUTE   m_videoAttr;
    AVIN_AUDIO_ATTRIBUTE   m_audioAttr;
}AVIN_APP_ATTRIBUTE, *PAVIN_APP_ATTRIBUTE;
...
*   
*   @param[in]  hInst AVIN instance handle
*   @param[in]  eAChannel will open this fast AChannel
*
*   @return RESULT define in avin.h
**/
AVINAPI RESULT  AVIN_OpenRDS(HAVINST hInst, E_AVIN_SOURCE_CHANNEL_T eAChannel);
...
*   
*   @param[in]  hInst AVIN instance handle
*   @param[in]  eAChannel will close this fast AChannel
*
*   @return RESULT define in avin.h
**/
AVINAPI RESULT  AVIN_CloseRDS(HAVINST hInst, E_AVIN_SOURCE_CHANNEL_T eAChannel);


AVINAPI RESULT AVIN_ConfigPortUsage();

AVINAPI RESULT AVIN_SetAppAttribute(UINT32 u4AppId, PAVIN_APP_ATTRIBUTE v_AppAttr);

AVINAPI RESULT AVIN_ChangeATVSignalMode(HAVINST hInst, UINT32 u4Mode);
#ifdef __cplusplus
}
#endif
