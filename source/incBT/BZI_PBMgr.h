...
#ifndef MAX_KEY_STRING
#define MAX_KEY_STRING 16
#endif
...
HRESULT BZ_PBMGRGetDevModel(wchar_t *szManufacturer, DWORD u4MaxManufacturerLen, wchar_t *szModel, DWORD u4MaxModelLen);

HRESULT BZ_PBMGRGetRecsPBSorted(UINT32 u4Index, P_PBMGRRECORD_T *ppRecord);

HRESULT BZ_PBMGRPYSearchPB(wchar_t (&wszKeyStr)[MAX_KEY_STRING], UINT32 *pu4ResultStartIdx, UINT32 *pu4ResultCnt);

HRESULT BZ_PBMGRSetPreferredProfile(PBMGR_TYPE_CONNECT eConnectType);

HRESULT BZ_PBMGRSetPBAPMethod(BOOL fgUsePullPhoneBook);
#ifdef __cplusplus
}
#endif
