...
LONG CalendarData::CalcDateDiff(WORD iEndYear, WORD iEndMonth, WORD iEndDay,
...
void  CalendarData::CalcLunarDate(WORD &iYear, WORD &iMonth ,WORD &iDay, LONG iSpanDays)
{
...
WORD CalendarData::GetLunarDate(WORD iYear, WORD iMonth, WORD iDay,
...
WORD  CalendarData::GetLunarHolDay(WORD iYear, WORD iMonth, WORD iDay)
{
...
void CalendarData::GetDate(WORD &iYear, WORD &iMonth, WORD &iDay) 
{
	iYear  = m_iYear;
	iMonth = m_iMonth;
	iDay   = m_iDay;
...
BOOL CalendarData::SetDate(WORD iYear, WORD iMonth, WORD iDay)
{
...
WORD CalendarData::WeekDay(WORD iYear, WORD iMonth, WORD iDay)
{
...
//add by wll
int CalendarData::ReturnWeekDay( unsigned int iYear, unsigned int iMonth, unsigned int iDay )  
{  
	int iWeek = 0;  
...
WORD CalendarData::MonthDays(WORD iYear, WORD iMonth)
{
	switch(iMonth)
	{
...
WORD CalendarData::GetLeapMonth(WORD iLunarYear)
{
	BYTE &flag = gLunarMonth[(iLunarYear - START_YEAR)/2];
	return  (iLunarYear - START_YEAR)%2 ? flag&0x0f : flag>>4;
}

LONG CalendarData::LunarMonthDays(WORD iLunarYear, WORD iLunarMonth)
{
...
WORD CalendarData::LunarYearDays(WORD iLunarYear)
{
	/*
	WORD days=348 ; //12*29
	int  month = 12 ;
...
void CalendarData::FormatLunarYear(WORD  iYear, LPTSTR pBuffer)
{	
	wchar_t szText1[]=L"甲乙丙丁戊己庚辛壬癸";
	wchar_t szText2[]=L"子丑寅卯辰巳午未申酉戌亥";
	wchar_t szText3[]=L"鼠牛虎兔龙蛇马羊猴鸡狗猪"; 
	//zxj
#if 1
	wcsncpy( pBuffer,szText1+((iYear-4)%10),1);
	wcsncpy( pBuffer+1,szText2+((iYear-4)%12),1);
	wcsncpy( pBuffer+2,_T(" "),1);
	wcsncpy( pBuffer+3,szText3+((iYear-4)%12),1);
	wcsncpy( pBuffer+4,L"年",1);
	...
	wcscpy(pBuffer+7,L"年");
#endif

}

void CalendarData::FormatMonth(WORD iMonth, LPTSTR pBuffer, BOOL bLunar)
{
	if(!bLunar && iMonth==1)
	{
		wcscpy(pBuffer, L"　一月");
...
#if 1
		wcsncpy(pBuffer, L" ", 1);
		wcsncpy(pBuffer+1, szText + (iMonth -1), 1);
		wcsncpy(pBuffer+2 , L"月",1);
		pBuffer[3] = '\0';
#else
		memcpy(pBuffer, L"　", 2);
		memcpy(pBuffer+2, szText + (iMonth -1)*2, 2);
		wcscpy(pBuffer+4 , L"月");
#endif
		return;
	}
	if (iMonth == 11)
...
void CalendarData::FormatLunarDay(WORD  iDay, LPTSTR pBuffer)
{
	...
	if(iDay != 20 && iDay !=30)
	{
		wcsncpy( pBuffer,szText1+ (iDay-1)/10, 1 );
		wcsncpy( pBuffer+1,szText2 + ((iDay-1)%10), 1 );
		pBuffer[2]='\0';
	}
	else
	{
		wcsncpy( pBuffer,szText1 + iDay/10, 1 );
		wcsncpy( pBuffer+1, szText2 +9, 1 );
		pBuffer[2]='\0';
	}
#else
	if(iDay != 20 && iDay !=30)
